...
describe('CarsList component', () => {
  it('should render list of elements correctly when list is not empty', () => {
    const {getAllByTestId} = render(<CarsList list={carsFixture} page={1} onPageSelect={jest.fn()}/>);
  ...
  it('should return null if  cars list is empty', () => {
    const {queryAllByTestId} = render(<CarsList list={{cars: []} as unknown as CarsListDTO} page={1} onPageSelect={jest.fn()}/>);
  ...
  it('should call onPageSelect when click on Pagination button', () => {
    ...
    const {getByText} = render(<CarsList list={carsFixture} onPageSelect={pageSelectHandler} page={1}/>);
